...
public class CompositeFilterView extends ViewPart {
	...
	public void createPartControl(Composite composite) {
		...
		nodesCheckbox = new Button(taggedWithAnyGroup, SWT.CHECK);
		nodesCheckbox.setFont(SWTResourceManager.getFont(".SF NS Text", 11, SWT.NORMAL));
		nodesCheckbox.setEnabled(false);
		nodesCheckbox.setText("Nodes: ");
		
		nodesText = new Text(taggedWithAnyGroup, SWT.BORDER);
		nodesText.setToolTipText("A comma seperated list of tag values. Raw values or XCSG common names such as XCSG.ControlFlow_Node can be entered.");
		nodesText.setEnabled(false);
		nodesText.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		
		edgesCheckbox = new Button(taggedWithAnyGroup, SWT.CHECK);
		edgesCheckbox.setFont(SWTResourceManager.getFont(".SF NS Text", 11, SWT.NORMAL));
		edgesCheckbox.setEnabled(false);
		edgesCheckbox.setText("Edges: ");
		
		edgesText = new Text(taggedWithAnyGroup, SWT.BORDER);
		edgesText.setToolTipText("A comma seperated list of tag values. Raw values or XCSG common names such as XCSG.ControlFlow_Edge can be entered.");
		edgesText.setEnabled(false);
		edgesText.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		...
		taggedRootsetRadio.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				// enable controls
				nodesCheckbox.setEnabled(isSelected);
				edgesCheckbox.setEnabled(isSelected);
				nodesTaggedWithAllCheckbox.setEnabled(isSelected);
				edgesTaggedWithAllCheckbox.setEnabled(isSelected);
				nodesText.setEnabled(isSelected);
				edgesText.setEnabled(isSelected);
		...
		nodesCheckbox.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
		...
		edgesCheckbox.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				if(taggedRootsetRadio.getSelection()){
					selectedRootset = getTaggedRootset().eval();
		...
		nodesText.addKeyListener(new KeyAdapter() {
			@Override
			public void keyReleased(KeyEvent key) {
				nodesCheckbox.setSelection(!nodesText.getText().isEmpty());
		...
		edgesText.addKeyListener(new KeyAdapter() {
			@Override
			public void keyReleased(KeyEvent key) {
				edgesCheckbox.setSelection(!edgesText.getText().isEmpty());
				if(key.character == '\r' || key.character == ','){
					selectedRootset = getTaggedRootset().eval();
					refreshRootset();
				}
	...
	private Q getTaggedRootset(){
		...
		if(nodesCheckbox.getSelection()){
			String selection = nodesText.getText();
			String[] tags = getTags(selection);
			if(tags != null && tags.length > 0){
				nodes = Query.universe().nodes(tags);
			}
		}
		...
		if(nodesTaggedWithAllCheckbox.getSelection()){
			...
			String[] tags = getTags(selection);
			if(tags != null && tags.length > 0){
				nodesTaggedWithAll = Query.universe().nodesTaggedWithAll(tags);
			}
		}
		Q edges = Common.empty();
		if(edgesCheckbox.getSelection()){
			String selection = edgesText.getText();
			String[] tags = getTags(selection);
			if(tags != null && tags.length > 0){
				edges = Query.universe().edges(tags);
			}
		}
		...
		if(edgesTaggedWithAllCheckbox.getSelection()){
			...
			String[] tags = getTags(selection);
			if(tags != null && tags.length > 0){
				edgesTaggedWithAll = Query.universe().edgesTaggedWithAll(tags);
			}
		}
		return nodes.union(nodesTaggedWithAll, edges, edgesTaggedWithAll);
	}
